  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="theme-icon-button flex cursor-pointer items-center gap-2 rounded-full border border-brown/8 px-4 py-3 text-[12px] uppercase tracking-[0.16em] transition-colors hover:bg-white"
      ...
      {isOpen && (
        <div
          className="absolute end-0 top-full z-50 mt-2 min-w-[170px] overflow-hidden rounded-[20px] border border-brown/8 bg-[rgb(var(--storefront-surface-raised-rgb)/0.97)] shadow-[0_18px_36px_rgb(var(--storefront-primary-rgb)/0.1)] backdrop-blur"
          role="listbox"
          aria-label={currentLocaleEntry.label}
        >
          {locales.map((locale) => (
