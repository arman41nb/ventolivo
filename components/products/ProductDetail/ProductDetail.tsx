  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-[3rem]">
      ...
      <div className="flex flex-col justify-center">
        ...
        <h1 className="font-serif text-[42px] leading-[1.1] text-dark mb-[1rem]">
          {product.name}
        </h1>
        <p className="text-[14px] leading-[1.8] text-muted mb-[1.5rem]">
          {product.description}
        </p>
        {product.weight && (
          <p className="text-[13px] text-muted mb-[0.5rem]">Weight: {product.weight}</p>
        )}
        {product.ingredients && product.ingredients.length > 0 && (
          <div className="mb-[1.5rem]">
            <p className="text-[12px] tracking-[1px] uppercase text-olive mb-[0.5rem]">
              Ingredients
            </p>
            <p className="text-[13px] text-muted">{product.ingredients.join(", ")}</p>
          </div>
        )}
        <p className="font-serif text-[28px] text-brown mb-[1.5rem]">
