...
export default function ProductCard({
  ...
  const buyHref = locale ? localePath(locale, `/buy/${product.slug}?source=card`) : `/buy/${product.slug}?source=card`;
  const media = getPrimaryProductMedia(product);

  return (
    <article
      className="theme-card-surface group relative overflow-hidden rounded-[28px] border border-brown/8 transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_22px_42px_rgb(var(--storefront-primary-rgb)/0.12)]"
    ...
    >
      ...
      <Link href={productHref} className="block">
        <div className="relative aspect-[0.92] overflow-hidden bg-[linear-gradient(135deg,color-mix(in_srgb,var(--storefront-surface-alt)_78%,white_22%),color-mix(in_srgb,var(--storefront-surface-alt)_64%,var(--storefront-primary)_36%))] about-image-drift">
          ...
          <span className="theme-soft-badge absolute right-4 top-4 z-10 rounded-full border border-brown/10 bg-[rgb(var(--storefront-surface-raised-rgb)/0.92)] px-3 py-2 text-[11px] uppercase tracking-[0.14em] luxe-badge-float">
            {product.tag}
          </span>
          {media?.type === "image" ? (
            <img
      ...
      <div className="p-6">
        ...
        <div className="mt-5 flex items-center justify-between gap-4">
          <span className="text-[1.1rem] font-bold text-brown">
